...
Обновлено 2026-09-22. **004-track-topology в работе**.
Предыдущий снимок: `003-track-environment` (тег `iteration-003`, завершена и проверена).
[Рабочий план 004](iterations/004-track-topology/PLAN.md). [Следующий этап](docs/next-iteration.md).
Текущая задача: T01 (реверс и анализ структур `.jnc` и `.edg`). Роли независимы от провайдера.
...
Геометрия трассы, материалы, текстурирование, расстановка пропов (.scn) и цилиндрический скайбокс горизонта (horz) проверены на треке `skidpad` через offscreen Vulkan и проверочные скрипты.
Ограничения активной итерации 004:
- Дорожные сплайны, осевая линия и границы полотна (`.edg`, `.jnc`) находятся в процессе реализации (T01–T06).
- Физические поверхности, карты коллизий и расчёт сцепления (`.map`, raycast) относятся к итерации 005.
- Трафик и анимированные объекты сценария пока не моделируются.
