...
async function loadTarget() {
  ...
  try {
    ...
    if (isTrack) {
      ...
      setStatus(`Трасса "${target}" загружена из ${sourceText}.${hasTopo ? " 3D границы включены (T)." : " Границы не найдены."}`);
    } else {
  ...
  if (isDriveCar) {
    ...
    if (rpmBar) {
      ...
      if (rpm > 0.82) {
        ...
        rpmBar.style.background = "linear-gradient(90deg, #10b981, #3b82f6)";
      }
    }
