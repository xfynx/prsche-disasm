  ...
  try {
    ...
    if (isTrack) {
      for (const file of selectedFiles) {
        ...
        // Include Sky/ directory .fsh files matching track name
        if (fullPath.includes("/sky/") && ext === "fsh" && stem === target) {
          names.push(selectedName(file));
          bytes.push(new Uint8Array(await file.arrayBuffer()));
          continue;
        }
        // Skip other sky files
        if (fullPath.includes("/sky/") || fullPath.includes("/sky new/")) continue;

        // Include .scn files matching track prefix (e.g. skidpad_st1.scn)
        if (ext === "scn" && stem.startsWith(target) &&
            !basename.includes("audio") && !basename.includes("camera") &&
        ...
        // Original: exact stem match for crp/fsh/env/edg/map/jnc
        if (
