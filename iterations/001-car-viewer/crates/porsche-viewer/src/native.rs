...
pub fn run_cli(args: impl IntoIterator<Item = String>) -> Result<(), String> {
    ...
    while let Some(arg) = iter.next() {
        match arg.as_str() {
            ...
            "--car" => car = next_value(&mut iter, "--car")?,
            "--screenshot" => {
    ...
    if paint_index != 0 && (command != "view" || screenshot.is_none()) {
        return Err("--paint-index requires view --screenshot".into());
    }
    let game_dir = game_dir.ok_or_else(|| format!("Missing --game-dir\n\n{}", usage()))?;
    let files = read_game_files(&game_dir, &car)?;
...
fn usage() -> String {
    "Usage:\n  porsche-viewer inspect --game-dir <path> --car 356a\n  porsche-viewer view --game-dir <path> --car 356a [--screenshot out.png] [--paint-index 0..5 (screenshot only)] [--yaw radians] [--pitch radians] [--width px] [--height px]".into()
}
...
mod gpu_diagnostics {
    ...
    fn paint_mask_gpu() {
        ...
        let white =
            pollster::block_on(render_pixels(&scene, 256, 256, Some(0.), Some(0.), 0)).unwrap();
        let red =
            pollster::block_on(render_pixels(&scene, 256, 256, Some(0.), Some(0.), 1)).unwrap();
    ...
    fn coplanar_material_bias_gpu() {
        ...
        let actual =
            pollster::block_on(render_pixels(&scene, 64, 64, Some(0.), Some(0.), 1)).unwrap();
        let mut lamp = scene.clone();
        lamp.meshes.retain(|mesh| mesh.material == 1);
        let expected =
            pollster::block_on(render_pixels(&lamp, 64, 64, Some(0.), Some(0.), 1)).unwrap();
        assert_eq!(
            actual, expected,
...
async fn render_screenshot(
    ...
    let pixels = render_pixels(scene, width, height, yaw, pitch, paint_index).await?;
